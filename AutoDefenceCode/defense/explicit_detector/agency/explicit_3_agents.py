...
class CoordinatorAgent(DefenseAssistantAgent):
    ...
    def generate_coordinate_reply(
        ...
        if last_agent_name == "TaskInputAgent":
            ...
            for group_manager, convo in self._oai_messages.items():
                for message in convo:
                    if message['name'] in ['Coordinator', 'IntentionAnalyzer']:
                        system_input += (f"{message['name']} ({message['role']}): {message['content']}")
